...
async def add_cache_headers(request: Request, call_next):
    """
    Browsercache für unveränderliche Bilder und statische Dateien.
    HTML bleibt revalidierbar, damit Statusänderungen sofort sichtbar werden.
    """
    response = await call_next(request)
    path = request.url.path

    if path.startswith("/data/screenshots/") or path.startswith("/data/diffs/"):
...
@app.get("/health")
def health():
    return {"status": "ok", "version": "7.0.0-beta.2", "recorder": "novnc"}
...
def dashboard(
    ...
    # Ungelesene Änderungen immer zuerst, innerhalb der Gruppen neueste zuerst.
    sites.sort(
        key=lambda site: (
            0 if site.unread_change else 1,
            -site.id,
        )
    )
...
def create_site(
    ...
    site = Site(
        ...
        monitor_selector=monitor_selector.strip(),
...
def edit_site(
    ...
    if site:
        ...
        site.monitor_selector = monitor_selector.strip()
